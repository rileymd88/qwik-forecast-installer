...
class ConnectQlik:
    ...
    def new_analytic_connection(self, name, host, port):
        """
        Creates a new analytic connection
        :param name: Name of analytic connection
        :param host: Host of analytic connection
        :param port: Port of analytic connection
        :returns: HTTP Status Code
        """
        path = 'qrs/analyticconnection'
        connection = {'name': name, 'host': host, 'port': port}
        data = json.dumps(connection)
        return self.post(path, data)

    def get_analytic_connection(self, filterparam=None, filtervalue=None):
        """
        Returns analytic connections
        :param filterparam: Property and operator of the filter
        :param filtervalue: Value of the filter
        :returns: JSON
        """
